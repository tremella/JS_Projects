...
const forbiddenDirs = ['node_modules']// dirs we don't want checked
    ...
    async collectFiles(targetPath) { // target path is the root folder.
        ...
        for (let file of files) { // = file or folder
            ...
            if (stats.isFile() && file.includes('.test.js')) {
                this.testFiles.push({name: filepath, shortName: file }); //array we want
            } else if (stats.isDirectory() && !forbiddenDirs.includes(file)) { //"if our file is a directory and isn't in a forbiddenDir,"
