...
router.post('/signin', [
    ...
    async (req, res) => {
     // let's find their old email and password within the req.body
        ...
        req.session.userId = user.id; // stored on cookie while signed in.
        res.redirect('/admin/products');
...
router.post('admin/products/:id/delete', requireAuth, async (req,res) => { //async because we reach into repo
